...
设计要点：
  ...
  · 单证类收敛（file_submit / file_withdraw）：
    **同一项目·同一批次·同一单证·同一天只保留一行最终态**。
    同一张单证当天反复勾选/取消，库里永远只有 1 行：kind 即最终动作（提交/撤销），
    created_at 即该次动作时间；跨天各留一行（报告可按天回溯）。
    勾选 → 覆盖为「提交」；取消 → 覆盖为「撤销」，不会同时存在两条。
    ⚠ v6.10 起收敛键**必须带 batch_id**：多批次下同名单证（如出口报关单）在
    同一天各提交一次，若不带批次会相互覆盖、丢掉其它批次的行。
  · 位移当日净收敛：node_shift 同项目·**同批次**·同节点·同日折叠为「当日净值」，净 0 不记；
    node_unshift（撤销）单独成条、永不与主动位移对消灭失。
  · 项目级单证的提交/撤销 batch_id 为空（scope='project'），按 `batch_id IS NULL` 收敛。
...
def _nets_of_day(project_id, kind, subject, day, batch_id=None):
    """取同项目·同批次·同主体·同日的位移行净天数累计（用于位移当日收敛）。

    必须带 batch_id：不同批次的同序节点 subject 完全相同（都叫"节点3"），
    不带批次会把两个批次的位移揉成一行、互相覆盖。
    """
    net = 0
    for row in db.get_op_log_range(project_id, start=day + " 00:00",
                                   end=day + " 23:59"):
        if row["kind"] == kind and (row["subject"] or "") == (subject or "") \
                and (row.get("batch_id") or None) == (batch_id or None):
...
def record(kind, project_id, node_id=None, subject="", detail="",
    ...
    v6.10 起「单证类」「位移类」的当日收敛键都**包含 batch_id**：
    同一天在 B01 与 B02 各提交一次《出口报关单》会各留一行（原先会互相覆盖删行）；
    项目级单证（batch_id 为空）按 `batch_id IS NULL` 收敛。
    """
    ...
    # ── 单证提交/撤交：同项目·同批次·同单证·同日收敛为一行最终态 ──
    ...
    # ── 主动位移：当日净收敛（净 0 不记；按批次分别收敛）──
    if kind == "node_shift":
        prev = _nets_of_day(project_id, "node_shift", subject, day, batch_id)
